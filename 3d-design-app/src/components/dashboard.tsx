...
function Dashboard({ currentUser }: any) {
    ...
    const [alert, setAlert] = useState<alertType>({ state: false, display: 'none' })
    ...
    const designAndUserData = useUserData(userId, userId)
    console.log(designAndUserData.currentUserMethod)
            ...
            <div className="absolute top-0 w-full h-[150vh] flex items-center flex-col">
                ...
                <div className="max-w-[66rem] my-[6rem] flex gap-y-12 gap-x-12 flex-wrap ">
                        <div className="bg-white rounded-lg shadow-xl h-[15rem] w-[20rem] flex justify-center items-center cursor-pointer" id="doc"
                            onClick={() => updateDesign(userId, 'add', '', '')}>
...

                        {
                            designAndUserData.designData.map((docCard: any) => {
                                return <DocumentCard key={docCard.docId} userId={userId} docId={docCard.docId} docName={docCard.docName} />
                            })
                        }
                </div>
                ...
                <div className="h-[20rem] w-[40rem] flex mt-[6rem]">
                    ...
                    <div className="h-full flex items-center">
                        <div className="flex flex-col h-[11rem] justify-evenly">
                            <Link href={`/profile/${userId}=${designAndUserData.currentUserName}`} className="userButton bg-white flex justify-center items-center">Show Profile</Link>
